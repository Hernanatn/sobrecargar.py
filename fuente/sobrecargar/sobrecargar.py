...
class _sobrecargar(metaclass=_SobrecargaDiferida):
    ...
    def __init__(self,función : Llamable,*, cache : bool = False, debug : bool = False) -> None:
        ...
        self.__cache : Opcional[dict[tuple[tuple[type[Cualquiera], ...], tuple[tuple[str, type[Cualquiera]]]], Llamable[..., Cualquiera]]] = {} if cache else None
        self.__debug = lambda msj: print(f"[DEBUG] {msj}") if debug else lambda msj: Nonee
