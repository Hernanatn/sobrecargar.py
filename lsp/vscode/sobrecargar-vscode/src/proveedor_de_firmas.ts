...
import { ParseadorSobrecargar, FirmaFuncion } from './parseador';

export class ProveedorFirmasSobrecargadas implements vscode.HoverProvider, vscode.SignatureHelpProvider {
    private parser: ParseadorSobrecargar;
    
    constructor(parser: ParseadorSobrecargar) {
        this.parser = parser;
    }
    ...
    public provideHover(
        ...
        for (const signature of firmas) {
            ...
            mainMarkdown.appendCodeblock(
                `def ${signature.nombre}(${this.formatearParametros(signature.cadena_parametros)})${signature.tipo_retorno ? ` -> ${signature.tipo_retorno}` : ''}`, 
                'python'
            );
    ...
    // Formato parametros
    private formatearParametros(paramString: string): string {
        // nueva linea?
    ...
    public provideSignatureHelp(
        ...
        signatureHelp.signatures = firmas.map((sig, index) => {
            ...
            const totalSignatures : number = firmas.length;
            ...
            const firma = new vscode.SignatureInformation(
                etiqueta_firma,
                new vscode.MarkdownString(documentacion)
            );
    ...
    private formatearDocstring(firma: FirmaFuncion): string {
        if (!firma.docstring) {
            return '';
        }
    ...
    private sanitizarDocstring(docstring: string): string {
        let limpia = docstring.trim();
        limpia = limpia.split('\n').map(line => line.trim()).join('\n');
        return limpia;
    }
    
    private extraerInfoParametros(param: string, docstring?: string): string | undefined {
        if (!docstring) {
            return undefined;
        }
    ...
    private contarComasDesbalanceadas(text: string): number {
        let contador = 0;
        let nivel_parentesis = 0;
    ...
    private parsearParametros(paramString: string): string[] {
        if (!paramString.trim()) {
            return [];
        }
